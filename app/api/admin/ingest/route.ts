 ...
 *   subcategorySlug string optional (defaults to the category's "General" sub-category)
 *   visibility   string  optional  ('public'|'private' — omit to inherit from category/sub-category)
...
export async function POST(req: NextRequest) {
  ...
  if (visibility != null && !VALID_VISIBILITY.includes(visibility)) {
    return NextResponse.json(
      { error: `visibility must be one of: ${VALID_VISIBILITY.join(', ')}, or omitted to inherit.` },
      { status: 400 }
    )
  }
  ...
  // ── Resolve sub-category — every article requires one. If the caller (e.g. an
  // automated Vector MCP ingest) doesn't know about sub-categories yet, fall back
  // to the category's default "General" sub-category created by migration 004. ──
  let resolvedSubcategorySlug = subcategorySlug?.trim()
  if (resolvedSubcategorySlug) {
    const subCheck = await pool.query(
      'SELECT slug FROM nuvho_kb.subcategories WHERE slug = $1 AND category_slug = $2',
      [resolvedSubcategorySlug, categorySlug.trim()]
    )
    if (subCheck.rowCount === 0) {
      return NextResponse.json({ error: 'Sub-category not found in that category.' }, { status: 404 })
    }
  } else {
    const defaultSub = await pool.query(
      `SELECT slug FROM nuvho_kb.subcategories WHERE category_slug = $1 ORDER BY sort_order, title LIMIT 1`,
      [categorySlug.trim()]
    )
    if (defaultSub.rowCount === 0) {
      return NextResponse.json(
        { error: 'This category has no sub-categories yet. Create one first or pass subcategorySlug explicitly.' },
        { status: 400 }
      )
    }
    resolvedSubcategorySlug = defaultSub.rows[0].slug
  }
